...
const sql = "INSERT INTO categories (`name`, `description`,`price`,`image`,`date`) VALUES (?)";
...
const getcategories = async (req, res) => {
    ...
    const sql = "SELECT * FROM categories"; // Make sure your table name is correct
...
const update = async (req,res) =>{
  ...
  console.log(imageurl)
      ...
      const db = await connectoDatabase()
const sql = "update  categories set `name`= ? ,`description` = ?, `price` =? ,`image`=? ,`data`=? ,where id = ?"
// const sql = "INSERT INTO categories (`name`, `description`,`price`,`image`,`date`) VALUES (?)";

const id = req.params.id;
await db.query(sql, [...productData, id], (err, result) => {
if (err) {
  console.error(err.message);
  return res.status(500).json({ success: false, message: "Database insertion error" });
}
...
console.error(error);
  ...
  export {getcategories, insertcategory ,update}
